...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // ── Loaders (دوال التحميل) ──────────────────────────────────────────────

    fun loadLiveCategories() {
        val creds = currentCreds ?: return
        _liveCategories.value = Resource.Loading()
        viewModelScope.launch { _liveCategories.value = repository.getLiveCategories(creds) }
    }

    fun loadLiveStreams(categoryId: String? = null) {
        val creds = currentCreds ?: return
        _liveStreams.value = Resource.Loading()
        viewModelScope.launch { _liveStreams.value = repository.getLiveStreams(creds, categoryId) }
    }

    fun loadAllLiveStreams() { loadLiveStreams(null) }

    fun loadVodCategories() {
        val creds = currentCreds ?: return
        _vodCategories.value = Resource.Loading()
        viewModelScope.launch { _vodCategories.value = repository.getVodCategories(creds) }
    }

    fun loadVodStreams(categoryId: String? = null) {
        val creds = currentCreds ?: return
        _vodStreams.value = Resource.Loading()
        viewModelScope.launch { _vodStreams.value = repository.getVodStreams(creds, categoryId) }
    }

    fun loadSeriesCategories() {
        val creds = currentCreds ?: return
        _seriesCategories.value = Resource.Loading()
        viewModelScope.launch { _seriesCategories.value = repository.getSeriesCategories(creds) }
    }

    fun loadSeries(categoryId: String? = null) {
        val creds = currentCreds ?: return
        _seriesList.value = Resource.Loading()
        viewModelScope.launch { _seriesList.value = repository.getSeries(creds, categoryId) }
    }

    fun loadSeriesInfo(seriesId: Int) {
        val creds = currentCreds ?: return
        _seriesInfo.value = Resource.Loading()
        viewModelScope.launch { _seriesInfo.value = repository.getSeriesInfo(creds, seriesId) }
    }
